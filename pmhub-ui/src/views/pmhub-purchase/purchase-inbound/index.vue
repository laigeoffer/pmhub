    ...
    <el-card shadow="never" class="search-wrapper">
      <el-form ref="searchFormRef" :inline="true" :model="searchData">
        <el-form-item label="单据类型">
          <el-input placeholder="采购入库" disabled />
        </el-form-item>
    ...
    <el-dialog :title="diaTitle" :visible.sync="createDialogVisible" width="90%" :before-close="handleClose">
      ...
      >
        <el-row :gutter="20">
          <el-col :span="4">
            <el-form-item label="单据类型">
              <el-input placeholder="采购入库" disabled style="width: 220px" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
        ...
        <el-row :gutter="20">
          <el-col :span="4">
            <el-form-item prop="totalPrice" label="金额合计">
              <el-input v-model="formData.totalPrice" disabled style="width: 220px" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item prop="arrears" label="欠款">
              <el-input-number v-model="formData.arrears" :min="0" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item prop="paymentDays" label="账期">
              <el-input-number v-model="formData.paymentDays" :min="0" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="4">
            <el-form-item prop="principalName" label="责任人">
              <el-input v-model="formData.principalName" style="width: 202px" readonly>
...
import {
  ...
  approvalSetApi,
  startProcessApi,
} from "@/api/pmhub-purchase/purchase-inbound.js"
...
export default {
    ...
    "formData.materials": {
      handler(newVal) {
        ...
        for (let i = 0; i < newVal.length; i++) {
          ...
          const unitPrice = newVal[i].info?.unitPrice
  ...
  methods: {
    ...
    async getApprovalSet() {
      // 查询审批设置数据
      const res = await getApprovalSetApi("PURCHASE_INTO")
    ...
    handleUpdate(row) {
      ...
      setTimeout(() => {
        ...
        this.formData.arrears = row.arrears
        this.formData.paymentDays = row.paymentDays
      ...
      this.$refs["formRef"].validate((valid) => {
        if (valid) {
          if (this.updateId === undefined) {
            createPurchaseApi(this.formData).then(() => {
              this.$modal.msgSuccess("新增成功")
              this.handleClose()
            })
          } else {
            updatePurchaseApi({ id: this.updateId, ...this.formData }).then(() => {
