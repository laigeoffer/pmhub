...
import com.laigeoffer.projectaihub.common.constant.Constants;
import com.laigeoffer.projectaihub.common.core.domain.AjaxResult;
import com.laigeoffer.projectaihub.common.core.domain.entity.SysMenu;
import com.laigeoffer.projectaihub.common.core.domain.entity.SysUser;
import com.laigeoffer.projectaihub.common.core.domain.model.LoginBody;
...
import com.laigeoffer.projectaihub.common.exception.file.InvalidExtensionException;
import com.laigeoffer.projectaihub.common.utils.OAUtils;
import com.laigeoffer.projectaihub.common.utils.SecurityUtils;
import com.laigeoffer.projectaihub.common.utils.WorkWxUtil;
import com.laigeoffer.projectaihub.common.utils.ZipDownloadUtil;
...
import com.laigeoffer.projectaihub.common.utils.oa.dto.OAUserDTO;
import com.laigeoffer.projectaihub.framework.web.service.SysLoginService;
import com.laigeoffer.projectaihub.framework.web.service.SysPermissionService;
import com.laigeoffer.projectaihub.system.domain.PaihOAuth2;
import com.laigeoffer.projectaihub.system.domain.PaihOAuth2Agree;
import com.laigeoffer.projectaihub.system.domain.PaihOAuth2Token;
...
import com.laigeoffer.projectaihub.system.service.ISysMenuService;
...
import static com.laigeoffer.projectaihub.common.core.domain.AjaxResult.success;
...
public class SysLoginController {
    ...
    @Value("${projectaihub.workWx.host}")
    String host;

    @Value("${projectaihub.workWx.corpid}")
    String corpid;

    @Value("${projectaihub.workWx.agentid}")
    String agentid;

    @Value("${projectaihub.workWx.path}")
    String path;
    ...
    public AjaxResult ssoForWorkWxCreate(@CookieValue(name = "user_info") String userInfo
        ...
        try {
            ...
            if (re>0){
                // 上传用户头像
                String avatar = FileUploadUtils.upload(ProjectaihubConfig.getAvatarPath(), FileUploadUtils.getMultipartFile(avatarFile), MimeTypeUtils.IMAGE_EXTENSION);
     ...
     *
     * @param paihOAuth2 OAurth2 参数
     * */
    @GetMapping("/sso/oauth2/authorize")
    public AjaxResult oauth2Authorize(HttpServletRequest request, PaihOAuth2 paihOAuth2) throws IOException {
        // 判断用户是否已经授权本应用获取用户信息进行单点登录
        if (ioAuth2Service.isAgree(SecurityUtils.getUserId(), paihOAuth2.getClient_id())) {
            // 用户已授权，重定向到授权页面并附带code
            String redirectUrl = paihOAuth2.getAuthorizeRedirectUrl(ioAuth2Service.createCode(SecurityUtils.getUserId()));
            return success(redirectUrl);
        } else {
            // 用户未授权，构建授权URL
            String requestUrl = request.getRequestURL().toString();
            // 检查url是否带参数，有就附带到授权页面去
            String[] urlParts = requestUrl.split(PaihOAuth2.SPLIT_STR);
            String url = urlParts.length > 1 ? PaihOAuth2.AUTHORIZE_URL + urlParts[1] : PaihOAuth2.AUTHORIZE_URL;
            return success(url);
        }
    }
     ...
     * 允许授权进行单点登录
     *
     * @param paihOAuth2Agree 包含clientid 对此id的应用进行授权
     * @return {@link AjaxResult}
     */
    @PostMapping("/sso/oauth2/agree")
    public AjaxResult oauth2Agree(@RequestBody PaihOAuth2Agree paihOAuth2Agree) {
        ioAuth2Service.agree(SecurityUtils.getUserId(), paihOAuth2Agree.getClientId());
        return success();
    }
    ...
    public JSONObject oauth2accessToken(PaihOAuth2 paihOAuth2) throws IOException {
        // 鉴定client_id 和 client_secret 通过后返货token
        if (ioAuth2Service.checkClientSecret(paihOAuth2.getClient_id(), paihOAuth2.getClient_secret())){
            PaihOAuth2Token paihOAuth2Token = new PaihOAuth2Token();
            String token = ioAuth2Service.createToken(paihOAuth2.getCode());
            if (ObjectUtil.isNotEmpty(token)){
                paihOAuth2Token.setAccess_token(token);
                return JSONUtil.parseObj(paihOAuth2Token);
            }else {
    ...
    public JSONObject oauth2user(HttpServletRequest request) throws IOException {
        ...
        //TOKEN 鉴权与用户信息返回
        PaihOAuth2User user = ioAuth2Service.getUser(token);
