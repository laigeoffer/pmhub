package com.laigeoffer.projectaihub.project.job;
...
import com.laigeoffer.projectaihub.project.domain.ProjectTask;
import com.laigeoffer.projectaihub.project.domain.ProjectTaskNotify;
import com.laigeoffer.projectaihub.project.mapper.ProjectTaskNotifyMapper;
import com.laigeoffer.projectaihub.project.mapper.ProjectTaskMapper;
            ...
            if (projectTask != null && projectTask.getCloseTime() != null) {
                ...
                if (ChronoUnit.DAYS.between(localDate, closeDate) < 0L) {
                    if (!ProjectTaskStatusEnum.FINISHED.getStatus().equals(projectTask.getExecuteStatus())) {
                        ...
                        // 设置任务详情页
                        String url = SsoUrlUtils.ssoCreate(appid, agentid, host + path + ssoPath + URLEncoder.encode(host + "/paih-project/my-task/info?taskId=" + projectTaskNotify.getTaskId()));
                        ...
                        taskOvertimeRemindDTO.setLinkUrl(OAUtils.ssoCreate(host + "/paih-project/my-task/info?taskId=" + projectTaskNotify.getTaskId()));
